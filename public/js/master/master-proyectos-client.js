/**
 * MASTER PROYECTOS CLIENT v1
 * 
 * Cliente JavaScript canónico para la UI de Sistema de Proyectos en dominio MASTER.
 * 
 * REGLAS ABSOLUTAS:
...
(function() {
  ...
  window.__AP_MASTER_PROYECTOS_STAMP__ = `MASTER_PROYECTOS@${APP_VERSION}|BUILD=${BUILD_ID}|STAMP=${BUILD_TIMESTAMP}|FEATURES=projects-v1+tab1-order-pipeline+tab2-expanded+tab3-categories`;
  
  // Log STAMP siempre visible
  console.log('%c[MASTER][PROYECTOS][STAMP]', 'color: #00ff99; background: #001122; padding: 2px 4px; font-weight: bold;', 
    window.__AP_MASTER_PROYECTOS_STAMP__);

  // BOOT LOG único y global
  console.log('[BOOT][MASTER][Proyectos] JS cargado', {
    time: Date.now(),
  ...
  if (typeof window === 'undefined' || window.__AP_CONTEXT__ !== 'MASTER') {
    console.warn('[MasterProyectos] No ejecutando en contexto no-MASTER');
    return;
  }

  const rootContainer = document.getElementById('master-proyectos-root');
  if (!rootContainer) {
    console.warn('[MasterProyectos] Contenedor #master-proyectos-root no encontrado');
    return;
  }
   ...
   */
  async function init() {
    if (DEBUG) console.log('[MASTER][PROYECTOS][INIT] Inicializando...');
  ...
  function renderMainTabs() {
    ...
    const tabs = [
      { id: 'activos', label: 'Proyectos Activos' },
   ...
   */
  async function loadProyectosActivos() {
    try {
      if (DEBUG) console.log('[MASTER][PROYECTOS][TAB1] Cargando proyectos activos...');
      
      const result = await apiFetch('/master/api/projects/active');
      state.proyectosActivos = result.projects || [];
      
      // Aplicar ordenación jerárquica
      sortProjectsHierarchical();
      
      renderProyectosActivos();
      
      if (DEBUG) console.log('[MASTER][PROYECTOS][TAB1_RENDER_ACTIVE] Tab 1 renderizado');
    } catch (error) {
      console.error('[MasterProyectos] Error cargando proyectos activos:', error);
      showMessage('Error cargando proyectos activos: ' + error.message, 'error');
    }
  }
   ...
   */
  function sortProjectsHierarchical() {
    if (state.orderBy.length === 0) return;
    
    if (DEBUG) console.log('[MASTER][PROYECTOS][ORDER_PIPELINE_ACTIVE] Ordenando con', state.orderBy);
    
    state.proyectosActivos.sort((a, b) => {
  ...
  function handleColumnSort(key) {
    ...
    try {
      localStorage.setItem('ap_master_projects_orderBy_v1', JSON.stringify(state.orderBy));
    } catch (e) {
  ...
  /**
   * Renderiza la tabla de proyectos activos
   */
  function renderProyectosActivos() {
    ...
    if (state.proyectosActivos.length === 0) {
      const emptyMsg = document.createElement('p');
      emptyMsg.textContent = 'No hay proyectos activos';
    ...
    state.proyectosActivos.forEach(project => {
      const row = document.createElement('tr');
      row.style.cssText = 'border-bottom: 1px solid #334155;';
      row.dataset.projectStateId = project.id;
      row.dataset.studentId = project.student_id;
      ...
      checkbox.checked = state.selectedProjects.includes(project.id);
      checkbox.addEventListener('change', (e) => {
        if (e.target.checked) {
          if (!state.selectedProjects.includes(project.id)) {
            state.selectedProjects.push(project.id);
          }
        } else {
          state.selectedProjects = state.selectedProjects.filter(id => id !== project.id);
        }
      ...
      tdApodo.textContent = project.student_apodo || '-';
      tdApodo.addEventListener('click', () => {
        ...
        if (project.student_id) {
          loadStudentById(project.student_id);
        }
      });
      ...
      tdEmail.textContent = project.student_email || '-';
      ...
      tdCategory.textContent = project.category_name || '-';
      ...
      nameInput.type = 'text';
      nameInput.value = project.custom_name || project.base_name || '';
      ...
      nameInput.addEventListener('blur', () => {
        updateProjectState(project.id, { custom_name: nameInput.value || null });
      });
      ...
      descInput.addEventListener('blur', () => {
        updateProjectState(project.id, { description: descInput.value || null });
      });
      ...
      const healthColor = project.health_status === 'green' ? '#10b981' : project.health_status === 'yellow' ? '#f59e0b' : '#ef4444';
      healthBadge.textContent = project.health_status || 'green';
      ...
      tdDays.textContent = project.days_since_clean !== undefined ? `${project.days_since_clean} días` : '-';
      tdDays.style.cssText = 'padding: 0.75rem;';
      row.appendChild(tdDays);
      
      // Última limpieza
      const tdLastClean = document.createElement('td');
      if (project.last_cleaned_at) {
        const date = new Date(project.last_cleaned_at);
      ...
      recInput.value = project.recurrence_days || 30;
      recInput.min = '1';
      ...
      recInput.addEventListener('blur', () => {
        updateProjectState(project.id, { recurrence_days: parseInt(recInput.value, 10) || 30 });
      });
      ...
      cleanBtn.addEventListener('click', () => handleCleanProject(project.student_id, project.project_id));
  ...
  function updateCleanSelectedButton() {
    if (btnCleanSelected) {
      btnCleanSelected.disabled = state.selectedProjects.length === 0;
    }
  }

  /**
   * Actualiza estado de proyecto
   */
  async function updateProjectState(projectStateId, fields) {
    try {
      await apiFetch(`/master/api/projects/state/${projectStateId}`, {
        method: 'PATCH',
      ...
      console.error('[MasterProyectos] Error actualizando estado:', error);
      showMessage('Error actualizando: ' + error.message, 'error');
    }
  }

  /**
   * Maneja la limpieza de un proyecto
   */
  async function handleCleanProject(studentId, projectId) {
    try {
      await apiFetch('/master/api/projects/clean', {
        method: 'POST',
        body: JSON.stringify({ student_id: studentId, project_id: projectId })
      });
      
      showMessage('Proyecto limpiado correctamente', 'success');
      await loadProyectosActivos();
    } catch (error) {
      console.error('[MasterProyectos] Error limpiando proyecto:', error);
      showMessage('Error limpiando proyecto: ' + error.message, 'error');
    }
  }

  /**
   * Maneja la limpieza de proyectos seleccionados
   */
  async function handleCleanSelected() {
    if (state.selectedProjects.length === 0) {
      showMessage('No hay proyectos seleccionados', 'error');
      return;
    }
    
    try {
      await apiFetch('/master/api/projects/clean-bulk', {
        method: 'POST',
        body: JSON.stringify({ project_state_ids: state.selectedProjects })
      });
      
      showMessage(`${state.selectedProjects.length} proyectos limpiados`, 'success');
      state.selectedProjects = [];
      await loadProyectosActivos();
    } catch (error) {
      console.error('[MasterProyectos] Error limpiando seleccionados:', error);
      showMessage('Error limpiando proyectos: ' + error.message, 'error');
    }
  }

  /**
   * Maneja la limpieza de todos los proyectos
   */
  async function handleCleanAll() {
    if (!confirm('¿Estás seguro de limpiar TODOS los proyectos activos?')) {
      return;
    }
    
    try {
      await apiFetch('/master/api/projects/clean-all', {
        method: 'POST'
      });
      
      showMessage('Todos los proyectos limpiados', 'success');
      await loadProyectosActivos();
    } catch (error) {
      console.error('[MasterProyectos] Error limpiando todos:', error);
  ...
  async function loadStudents() {
    try {
      ...
      console.error('[MasterProyectos] Error cargando alumnos:', error);
      state.students = [];
    }
  }
  ...
  async function loadStudentById(studentId) {
    try {
      const result = await apiFetch(`/master/api/projects/student/${studentId}`);
      state.currentStudent = state.students.find(s => s.id === studentId) || { id: studentId };
      state.studentProjects = result.projects || [];
      renderStudentConfig(result);
    } catch (error) {
      console.error('[MasterProyectos] Error cargando alumno:', error);
  ...
  function renderStudentConfig(data) {
    ...
    limitSelect.addEventListener('change', async () => {
      ...
      try {
        const result = await apiFetch('/master/api/projects/limit', {
          method: 'POST',
          body: JSON.stringify({
        ...
        console.error('[MasterProyectos] Error actualizando límite:', error);
    ...
    createTitle.textContent = '➕ Crear nuevo proyecto';
    ...
    nameInput.placeholder = 'Ej: Proyecto de sanación, Proyecto familiar...';
    ...
    descTextarea.id = 'new-project-description';
    descTextarea.rows = 2;
    descTextarea.placeholder = 'Descripción del proyecto...';
    ...
    typeSelect.id = 'new-project-category';
    ...
    (async () => {
      try {
        const catResult = await apiFetch('/master/api/project-categories');
        ...
        console.error('[MasterProyectos] Error cargando categorías:', error);
    ...
    createBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        await apiFetch('/master/api/projects/create-for-student', {
          method: 'POST',
          body: JSON.stringify({
            student_id: state.currentStudent.id,
            name,
        ...
        showMessage('Proyecto creado y activado', 'success');
        ...
        // Refetch proyectos del alumno
        await loadStudentById(state.currentStudent.id);
      } catch (error) {
        console.error('[MasterProyectos] Error creando proyecto:', error);
        showMessage('Error creando proyecto: ' + error.message, 'error');
      } finally {
    ...
    if (data.projects && data.projects.length > 0) {
      ...
      projectsGrid.style.cssText = 'display: grid; grid-template-columns: repeat(auto-fill, minmax(400px, 1fr)); gap: 1rem;';
      
      data.projects.forEach(project => {
        const projectCard = document.createElement('div');
        projectCard.style.cssText = 'padding: 1rem; background: #1e293b; border: 1px solid #334155; border-radius: 0.5rem;';
        ...
        descTextarea.addEventListener('blur', () => {
          updateProjectState(project.id, { description: descTextarea.value || null });
        });
        
        // Tipo (read-only)
        const typeLabel = document.createElement('div');
        typeLabel.textContent = `Tipo: ${project.category_name || '-'}`;
        ...
        const healthBadge = document.createElement('span');
        const healthColor = project.health_status === 'green' ? '#10b981' : project.health_status === 'yellow' ? '#f59e0b' : '#ef4444';
        healthBadge.textContent = `Salud: ${project.health_status || 'green'}`;
        healthBadge.style.cssText = `padding: 0.25rem 0.5rem; background: ${healthColor}; color: white; border-radius: 0.25rem; font-size: 0.75rem; text-transform: uppercase;`;
        ...
        daysLabel.textContent = `Días desde limpieza: ${project.days_since_clean !== undefined ? `${project.days_since_clean} días` : '-'}`;
        ...
        statusBadge.textContent = project.is_active ? 'Activo' : 'Inactivo';
        statusBadge.style.cssText = `padding: 0.25rem 0.5rem; background: ${project.is_active ? '#10b981' : '#64748b'}; color: white; border-radius: 0.25rem; font-size: 0.75rem;`;
        ...
        toggleBtn.textContent = project.is_active ? 'Desactivar' : 'Activar';
        toggleBtn.style.cssText = `padding: 0.5rem 1rem; background: ${project.is_active ? '#f59e0b' : '#3b82f6'}; color: white; border: none; border-radius: 0.25rem; cursor: pointer; font-size: 0.875rem;`;
        toggleBtn.addEventListener('click', async () => {
          try {
            if (project.is_active) {
              await apiFetch('/master/api/projects/deactivate', {
                method: 'POST',
                body: JSON.stringify({ student_id: project.student_id, project_id: project.project_id })
              });
            } else {
              await apiFetch('/master/api/projects/activate', {
                method: 'POST',
                body: JSON.stringify({ student_id: project.student_id, project_id: project.project_id })
              });
            }
            showMessage(project.is_active ? 'Proyecto desactivado' : 'Proyecto activado', 'success');
            await loadStudentById(project.student_id);
          } catch (error) {
        ...
        projectCard.appendChild(nameLabel);
        projectCard.appendChild(nameInput);
        projectCard.appendChild(descLabel);
        projectCard.appendChild(descTextarea);
        projectCard.appendChild(typeLabel);
        projectCard.appendChild(healthLabel);
        projectCard.appendChild(lastCleanLabel);
        projectCard.appendChild(daysLabel);
        projectCard.appendChild(statusLabel);
        projectCard.appendChild(actionsDiv);
        
        projectsGrid.appendChild(projectCard);
      });
      
      studentConfigContainer.appendChild(projectsGrid);
    } else {
      const emptyMsg = document.createElement('p');
      emptyMsg.textContent = 'Este alumno no tiene proyectos configurados';
    ...
    if (DEBUG) console.log('[MASTER][PROYECTOS][TAB2_EXPANDED_RENDER_ACTIVE] Tab 2 renderizado');
  }
  ...
  async function loadCategories() {
    try {
      const result = await apiFetch('/master/api/project-categories');
  ...
  async function updateCategory(categoryId, fields) {
    try {
      await apiFetch(`/master/api/project-categories/${categoryId}`, {
        method: 'PATCH',
        body: JSON.stringify(fields)
      });
      ...
      console.error('[MasterProyectos] Error actualizando categoría:', error);
  ...
  async function handleCrearCategoria() {
    ...
    try {
      await apiFetch('/master/api/project-categories', {
        method: 'POST',
        body: JSON.stringify({
          name,
      ...
      console.error('[MasterProyectos] Error creando categoría:', error);
  ...
  try {
    const saved = localStorage.getItem('ap_master_projects_orderBy_v1');
    if (saved) {
