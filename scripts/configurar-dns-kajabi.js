// scripts/configurar-dns-kajabi.js
// Script para configurar los registros DNS de Kajabi en Cloudflare para eugenihidalgo.org
...
// Registros DNS requeridos por Kajabi
// NOTA: En las instrucciones de Kajabi, "kjbm" es el host, pero para eugenihidalgo.org
// debemos usar "@" (raíz del dominio) o el dominio completo
const KAJABI_RECORDS = {
  // TXT Records
  txt: [
    {
      name: '@', // Raíz del dominio (eugenihidalgo.org)
      content: 'v=spf1 include:mailgun.org ~all',
      description: 'SPF Record para Mailgun'
    },
    {
      name: 'k1._domainkey', // DKIM key
  ...
  cname: [
    {
      name: 'email', // email.eugenihidalgo.org
      content: 'mailgun.org',
...
async function listExistingRecords(zoneId, headers) {
  console.log(`\n🔍 Listando registros DNS existentes para ${DOMAIN}...`);
...
async function findExistingRecord(records, type, name) {
  ...
  const searchName = name === '@' ? DOMAIN : name.includes('.') ? name : `${name}.${DOMAIN}`;
  
  // Buscar registros que coincidan
  return records.find(r => {
    if (r.type !== type) return false;
    
    // Para registros en la raíz del dominio
    if (name === '@') {
      return r.name === DOMAIN || r.name === '@';
    }
    
    // Para subdominios, comparar nombres normalizados
    const recordName = r.name.endsWith(`.${DOMAIN}`) 
      ? r.name 
      : r.name === name 
        ? `${name}.${DOMAIN}` 
        : r.name;
    
    return recordName === searchName || recordName === name;
  });
}

async function createOrUpdateRecord(zoneId, headers, type, name, content, priority = null, existingRecord = null) {
  const fullName = name === '@' ? DOMAIN : name.includes('.') ? name : `${name}.${DOMAIN}`;
...
async function configureKajabiDNS(zoneId, headers, existingRecords) {
  ...
  console.log(`\n📝 Configurando registros DNS para Kajabi...\n`);
  ...
  for (const txtRecord of KAJABI_RECORDS.txt) {
    ...
    console.log(`   → ${txtRecord.description} (${txtRecord.name === '@' ? DOMAIN : `${txtRecord.name}.${DOMAIN}`})`);
...
async function main() {
  try {
    console.log('🚀 Configurando DNS de Cloudflare para Kajabi\n');
    console.log(`📋 Configuración:`);
    console.log(`   Dominio: ${DOMAIN}`);
    console.log(`   www: www.${DOMAIN}`);
    console.log(`\n📝 Registros a configurar:`);
    console.log(`   - 3 registros TXT (SPF, DKIM, DMARC)`);
    console.log(`   - 2 registros MX (mxa.mailgun.org, mxb.mailgun.org)`);
    console.log(`   - 1 registro CNAME (email → mailgun.org)\n`);
    ...
    const existingRecords = await listExistingRecords(zoneId, headers);
    ...
    console.log(`   3. Una vez verificado, Kajabi activará el dominio personalizado\n`);
    
    console.log(`🔍 Para verificar los registros:`);
    console.log(`   dig ${DOMAIN} TXT`);
    console.log(`   dig ${DOMAIN} MX`);
    console.log(`   dig email.${DOMAIN} CNAME\n`);
