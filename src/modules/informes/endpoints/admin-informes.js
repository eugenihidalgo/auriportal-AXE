// src/modules/informes/endpoints/admin-informes.js
...
export async function renderInformes(request, env) {
  try {
    const moduloActivo = await isActivo('informe_semanal');

    const content = `
      <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center mb-6">
          <div>
            <h2 class="text-2xl font-bold text-white mb-2">📝 Informes Semanales</h2>
            <p class="text-slate-400">Generación automática de informes semanales para alumnos</p>
          </div>
          <div>
            ${moduloActivo 
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-lg p-8 text-center">
          ...
          <h3 class="text-lg font-medium text-white mb-2">Informes Semanales - En Construcción</h3>
          <p class="text-slate-400">Resúmenes automáticos de prácticas, reflexiones y progreso</p>
        </div>
      </div>
    `;

    const html = replace(baseTemplate, { TITLE: 'Informes Semanales', CONTENT: content });
