// src/modules/sorpresas/endpoints/admin-sorpresas.js
...
export async function renderSorpresas(request, env) {
  try {
    const moduloActivo = await isActivo('practicas_sorpresa');

    const content = `
      <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center mb-6">
          <div>
            <h2 class="text-2xl font-bold text-white mb-2">🎁 Prácticas Sorpresa</h2>
            <p class="text-slate-400">Sistema de recomendación inteligente de prácticas</p>
          </div>
          <div>
            ${moduloActivo 
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-lg p-8 text-center">
          ...
          <h3 class="text-lg font-medium text-white mb-2">Prácticas Sorpresa - En Construcción</h3>
          <p class="text-slate-400">Sugerencias inteligentes basadas en comportamiento del alumno</p>
        </div>
      </div>
    `;

    const html = replace(baseTemplate, { TITLE: 'Prácticas Sorpresa', CONTENT: content });
    return new Response(html, { headers: { 'Content-Type': 'text/html; charset=UTF-8' } });
  } catch (error) {
    console.error('Error renderizando Sorpresas:', error);
