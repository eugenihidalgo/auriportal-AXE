// src/infra/repos/student-project-state-repo-pg.js
// Implementación PostgreSQL del Repositorio de Estado Alumno-Proyecto

import { query } from '../../../database/pg.js';

export class StudentProjectStateRepoPg {
  async getById(id, client = null) {
    ...
    const result = await queryFn(`
      ...
      FROM student_project_state sps
      JOIN projects_catalog pc ON sps.project_id = pc.id
  ...
  async getByStudentAndProject(studentId, projectId, client = null) {
    if (!studentId || !projectId) return null;
    ...
    const result = await queryFn(`
      ...
      JOIN projects_catalog pc ON sps.project_id = pc.id
      JOIN project_categories cat ON pc.category_id = cat.id
      WHERE sps.student_id = $1 AND sps.project_id = $2
    `, [studentId, projectId]);
    return result.rows[0] || null;
  }
  ...
  async create(data, client = null) {
    ...
    const queryFn = client ? client.query.bind(client) : query;
    const result = await queryFn(`
      INSERT INTO student_project_state (
        student_id, project_id, is_active, is_reviewed, custom_name, description, recurrence_days
  ...
  async updateById(id, patch, client = null) {
    ...
    const queryFn = client ? client.query.bind(client) : query;
    const result = await queryFn(
      `UPDATE student_project_state SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
      values
    );
  ...
  async countActiveByStudent(studentId, client = null) {
    ...
    const queryFn = client ? client.query.bind(client) : query;
    const result = await queryFn(
      'SELECT COUNT(*) as count FROM student_project_state WHERE student_id = $1 AND is_active = TRUE',
      [studentId]
    );
  ...
  async getOldestActiveByStudent(studentId, client = null) {
    ...
    const queryFn = client ? client.query.bind(client) : query;
    const result = await queryFn(
      'SELECT * FROM student_project_state WHERE student_id = $1 AND is_active = TRUE ORDER BY created_at ASC LIMIT 1',
      [studentId]
    );
    return result.rows[0] || null;
  }
...
export function getDefaultStudentProjectStateRepo() {
  if (!defaultRepo) {
    defaultRepo = new StudentProjectStateRepoPg();
  }
  return defaultRepo;
}
