...
// Importar funciones de validación (se cargan dinámicamente o desde window)
let getActionOrFailFn, validatePayloadFn, logContractViolationFn, getActionFn;

// Intentar importar si estamos en módulo ES6
if (typeof window === 'undefined' || window.__AP_UX_ACTION_SCHEMA__) {
  // Si está disponible en window, usarlo
  if (typeof window !== 'undefined' && window.__AP_UX_ACTION_SCHEMA__) {
    getActionOrFailFn = (id, registry) => window.__AP_UX_ACTION_SCHEMA__.validateExists(id, registry);
    logContractViolationFn = window.__AP_UX_ACTION_SCHEMA__.logViolation;
    validatePayloadFn = (id, payload, context) => {
      const registry = window.__AP_UX_ACTION_REGISTRY_CORE__ || window.__AP_UX_ACTION_REGISTRY__;
      if (!registry) return { valid: false, errors: ['Registry no disponible'] };
      const action = registry.get(id);
      if (!action) return { valid: false, errors: [`Acción ${id} no registrada`] };
      return window.__AP_UX_ACTION_SCHEMA__.validatePayload(action, payload, context);
    };
    getActionFn = (id) => {
      const registry = window.__AP_UX_ACTION_REGISTRY_CORE__ || window.__AP_UX_ACTION_REGISTRY__;
      return registry ? registry.get(id) : null;
    };
  } else {
    // Fallback: funciones básicas
    getActionOrFailFn = (id, registry) => {
      if (!registry || !registry.get) return { exists: false, action: null, error: 'Registry no disponible' };
      const action = registry.get(id);
      return action ? { exists: true, action, error: null } : { exists: false, action: null, error: `Acción ${id} no registrada` };
    };
    validatePayloadFn = () => ({ valid: true, errors: [] });
    logContractViolationFn = (id, violation, context) => {
      console.error('[UX_CONTRACT_VIOLATION]', { action_id: id, violation, context, timestamp: new Date().toISOString() });
    };
    getActionFn = (id) => {
      const registry = window.__AP_UX_ACTION_REGISTRY_CORE__ || window.__AP_UX_ACTION_REGISTRY__;
      return registry ? registry.get(id) : null;
...
export async function performAction({ action_id, payload = {}, context = {}, uiState = {} }) {
  ...
  if (!action_id || typeof action_id !== 'string') {
    ...
    logContractViolationFn(action_id || 'unknown', 'missing_action_id', { payload, context });
    throw new Error(error);
  }

  // Obtener registry (core primero, fallback a legacy)
  const registryCore = typeof window !== 'undefined' && window.__AP_UX_ACTION_REGISTRY_CORE__;
  const registryLegacy = typeof window !== 'undefined' && window.__AP_UX_ACTION_REGISTRY__;
  
  const registryWrapper = {
    get: (id) => {
      if (registryCore && registryCore.get) {
        return registryCore.get(id);
      }
      if (registryLegacy && registryLegacy.get) {
        return registryLegacy.get(id);
      }
      return null;
    }
  };
  ...
  const actionCheck = getActionOrFailFn(action_id, registryWrapper);

  if (!actionCheck.exists) {
    logContractViolationFn(action_id, 'action_not_registered', { payload, context });
  ...
  const validation = validatePayloadFn(action_id, payload, context);
  if (!validation.valid) {
    logContractViolationFn(action_id, 'invalid_payload', { payload, context, errors: validation.errors });
  ...
  try {
    ...
    logContractViolationFn(action_id, 'execution_error', {
      payload: finalPayload,
      context,
      error: error.message
    });
