 ...
 * @param {Object} [registry] - Registry de acciones (opcional, se obtiene de window si no se proporciona)
 * @returns {Object} { exists: boolean, action: Object|null, error: string|null }
 */
export function validateActionExists(action_id, registry = null) {
  ...
  // Si no se proporciona registry, obtener de window
  if (!registry) {
    registry = typeof window !== 'undefined' && window.__AP_UX_ACTION_REGISTRY_CORE__;
    if (!registry) {
      // Fallback a registry legacy
      registry = typeof window !== 'undefined' && window.__AP_UX_ACTION_REGISTRY__;
    }
  }
