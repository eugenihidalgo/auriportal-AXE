// src/endpoints/admin-energias-indeseables.js
// Endpoints del Admin Panel para Energías Indeseables

import { readFileSync } from 'fs';
import { fileURLToPath } from 'url';
import { dirname, join } from 'path';
import { 
  listarAspectosIndeseablesGlobales,
  crearAspectoIndeseableRapido,
  actualizarAspectoIndeseableDetalle,
  getAspectosIndeseablesAlumno,
  getEstadisticasLimpiezaIndeseables,
  getAlumnosPorAspectoIndeseable,
...
export async function renderEnergiasIndeseables(request, env) {
  const url = new URL(request.url);
  const action = url.searchParams.get('action');
  const alumnoId = url.searchParams.get('alumno_id');

  // POST: Crear energía indeseable rápida
  if (request.method === 'POST' && action === 'create_rapido') {
    try {
      ...
      const id = await crearAspectoIndeseableRapido(nombre);
  ...
  if (request.method === 'POST' && action === 'marcar_todos_limpios') {
    try {
      ...
      const resultado = await marcarTodosAlumnosLimpiosPorAspectoIndeseable(aspectoId);
  ...
  if (action === 'ver_alumnos') {
    ...
    const alumnos = await getAlumnosPorAspectoIndeseable(parseInt(aspectoId));
    const aspecto = (await query(`SELECT nombre FROM aspectos_indeseables WHERE id = $1`, [aspectoId])).rows[0];
            ...
            <div class="space-y-2 max-h-[60vh] overflow-y-auto">
              ${limpios.length > 0 ? limpios.map(al => `
                <div class="bg-green-900/20 rounded p-3 border border-green-800">
                  ...
                  <button onclick="marcarLimpioDirecto('indeseables', ${aspectoId}, ${al.alumno_id}, '${(al.apodo || al.email || '').replace(/'/g, "\\'")}')" 
        ...
        async function marcarLimpioDirecto(tipo, aspectoId, alumnoId, nombreAlumno) {
          try {
            ...
            if (response.ok && result.success) {
              ...
              if (modal) {
                const response2 = await fetch(\`/admin/energias-indeseables?action=ver_alumnos&aspecto_id=\${aspectoId}\`);
  ...
  // POST: Actualizar energía indeseable
  if (request.method === 'POST' && action === 'update') {
    try {
      ...
      await actualizarAspectoIndeseableDetalle(aspectoId, datos);
  ...
  // GET: Listar energías indeseables
  const aspectos = await listarAspectosIndeseablesGlobales();
  const aspectosAlumno = alumnoId ? await getAspectosIndeseablesAlumno(parseInt(alumnoId)) : null;
  const estadisticas = alumnoId ? await getEstadisticasLimpiezaIndeseables(parseInt(alumnoId)) : null;

  const content = `
    <div class="p-6">
      <h1 class="text-3xl font-bold text-white mb-6">⚠️ Energías Indeseables – Impurezas y Densidades</h1>
      ...
      <div class="bg-slate-800 rounded-lg p-6">
        <h2 class="text-xl font-semibold text-white mb-4">Energías Indeseables</h2>
        ...
        <div class="mb-6 bg-slate-700 rounded p-4">
          <form id="formCrearRapido" class="flex gap-2">
            <input 
              ...
              name="nombre"
              placeholder="Añadir nueva energía indeseable… (Enter para guardar)"
                    ...
                    <tr class="border-b border-slate-700 hover:bg-slate-700" data-id="${a.id}" data-nivel="${nivelActual}">
                      ...
                      <td class="py-3">
                        ...
                        >
                          ...
                          <option value="Normal" ${a.prioridad === 'Normal' ? 'selected' : ''}>Normal</option>
                        </select>
                      </td>
                      <td class="py-3">
                        <div class="flex gap-2">
                          ...
                          <button 
                            onclick="marcarTodosLimpiosAspecto(${a.id}, '${(a.nombre || '').replace(/'/g, "\\'")}')"
    ...
    <script>
      // Crear energía indeseable rápida
      document.getElementById('formCrearRapido').addEventListener('submit', async (e) => {
        ...
        if (!nombre || nombre === '') {
          alert('Por favor, introduce un nombre para la energía indeseable');
          input.focus();
          return;
        }
        ...
        try {
          ...
          if (res.ok && data.success) {
            ...
            alert('Error creando energía indeseable: ' + (data.error || 'Error desconocido'));
            submitBtn.disabled = false;
            submitBtn.textContent = originalText;
          }
        } catch (error) {
      ...
      async function actualizarAspecto(id, campo, valor) {
        try {
          ...
          if (!res.ok) {
            alert('Error actualizando energía indeseable');
            location.reload();
          } else {
      ...
      async function verAspectoAlumno(aspectoId) {
        const response = await fetch(\`/admin/energias-indeseables?action=ver_alumnos&aspecto_id=\${aspectoId}\`);
      ...
      async function marcarTodosLimpiosAspecto(aspectoId, nombreAspecto) {
        ...
        try {
          const response = await fetch('/admin/energias-indeseables?action=marcar_todos_limpios', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
