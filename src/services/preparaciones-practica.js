...
// Gestión de preparaciones para la práctica antes de limpiar
...
async function ejecutarMigracionSiEsNecesario() {
  ...
  try {
    ...
    const result = await query(`
      ...
      WHERE table_name = 'preparaciones_practica' AND column_name = 'tipo'
    `);
    
    if (result.rows.length === 0) {
      console.log('🔄 Ejecutando migración Fase 1: añadiendo columnas a preparaciones_practica...');
      ...
      console.log('✅ Migración Fase 1 completada para preparaciones_practica');
    }
...
/**
 * Lista todas las preparaciones activas, ordenadas por nivel y nombre
 * @returns {Promise<Array>}
 */
export async function listarPreparaciones() {
  try {
    const result = await query(`
      SELECT * FROM preparaciones_practica
      WHERE activo = true
      ORDER BY nivel ASC, nombre ASC
    `);
    return result.rows;
  } catch (error) {
    console.error('Error listando preparaciones:', error);
    return [];
  }
}

/**
 * Obtiene preparaciones disponibles para un nivel específico
 * @param {number} nivelAlumno - Nivel del alumno
 * @returns {Promise<Array>} Preparaciones con nivel <= nivelAlumno
 */
export async function obtenerPreparacionesPorNivel(nivelAlumno) {
  try {
    const result = await query(`
      SELECT * FROM preparaciones_practica
      WHERE activo = true
      ...
      ORDER BY 
        ...
        CASE WHEN nombre ILIKE 'Hall de la iniciación' THEN 0 ELSE 1 END,
        nivel ASC,
        nombre ASC
    `, [nivelAlumno]);
    return result.rows;
  } catch (error) {
    console.error('Error obteniendo preparaciones por nivel:', error);
    return [];
  }
}
 ...
 * @param {number} preparacionId
 * @returns {Promise<Object|null>}
 */
export async function obtenerPreparacion(preparacionId) {
  try {
    const result = await query(`
      SELECT * FROM preparaciones_practica
      WHERE id = $1
 ...
 * @param {Object} datos
 * @returns {Promise<number>} ID de la preparación creada
 */
export async function crearPreparacion(datos) {
  try {
    ...
    console.error('Error creando preparación:', error);
    throw error;
  }
}
 ...
 */
export async function actualizarPreparacion(preparacionId, datos) {
  try {
    // Ejecutar migración si es necesario
    await ejecutarMigracionSiEsNecesario();
    ...
    params.push(preparacionId);
    
    await query(`
      UPDATE preparaciones_practica
      SET ${updates.join(', ')}
      WHERE id = $${paramIndex}
    `, params);
    ...
    console.error('Error actualizando preparación:', error);
    return false;
  }
}

/**
 * Elimina una preparación (soft delete)
 * @param {number} preparacionId
 * @returns {Promise<boolean>}
 */
export async function eliminarPreparacion(preparacionId) {
  try {
    await query(`
      UPDATE preparaciones_practica
    ...
    console.error('Error eliminando preparación:', error);
    return false;
  }
}
